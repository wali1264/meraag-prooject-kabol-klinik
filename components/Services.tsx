...
const Services: React.FC = () => {
  ...
  const [packages, setPackages] = useState<Package[]>([
    {
      id: '1',
      title: 'پکیج حج VIP (۱۴۰۳)',
      type: 'حج',
      visa: { type: 'سعودی الکترونیک', cost: '۱۵,۰۰۰' },
      hotel: { name: 'زمزم پولمن', nights: '۱۵ شب', rooms: 'اتاق ۲ نفره' },
      food: { type: 'سه وقت', duration: '۱۵ روز' },
      flight: { route: 'کابل-جده-کابل', sellingPrice: '۵۲,۰۰۰' },
      profit: '۲۲,۰۰۰',
      totalPrice: '۱۳۹,۰۰۰',
      color: 'emerald'
    },
    {
      id: '2',
      title: 'عمره اقتصادی (۱۴ روزه)',
      type: 'عمره',
      visa: { type: 'عمره انفرادی', cost: '۱۲,۰۰۰' },
      hotel: { name: 'انوار مدینه', nights: '۱۴ شب', rooms: 'اتاق ۴ نفره' },
      food: { type: 'دو وقت', duration: '۱۴ روز' },
      flight: { route: 'هرات-مدینه-هرات', sellingPrice: '۴۲,۰۰۰' },
      profit: '۱۲,۴۰۰',
      totalPrice: '۸۵,۵۰۰',
      color: 'blue'
    }
    ...
    <div className="p-8 space-y-8 max-w-5xl mx-auto">
      ...
      <div className="flex justify-between items-start">
        <div className="flex flex-col gap-1">
          ...
          <p className="text-slate-500 font-medium">تعریف و تنظیم پکیج‌های استاندارد حج و عمره</p>
        </div>
      ...
      <div className="grid grid-cols-1 gap-8 pb-12">
        {packages.map((pkg) => (
          <div key={pkg.id} className="bg-white rounded-[40px] shadow-sm border border-slate-100 overflow-hidden flex flex-col transition-all hover:shadow-md">
            ...
            <div className="px-8 py-6 border-y border-slate-50 grid grid-cols-2 gap-y-10 gap-x-12">
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-slate-400 text-[10px] font-bold">({pkg.visa.cost} AFN)</p>
                </div>
              </div>
              ...
              <div className="flex items-start justify-between">
                <div className="space-y-1">
                  ...
                  <p className="text-emerald-500 text-[10px] font-black">قیمت فروش: AFN {pkg.flight.sellingPrice}</p>
                </div>
              </div>
            ...
            <div className="p-8 bg-slate-50/50 flex items-center justify-between">
              <div className="flex items-center gap-3 bg-white px-5 py-3 rounded-2xl border border-slate-100">
                ...
                <span className="text-lg font-black text-slate-800">AFN {pkg.profit}</span>
              </div>

              <div className="text-left flex flex-col items-end">
                <span className="text-[10px] font-bold text-slate-400 mb-1">قیمت نهایی برای مشتری</span>
                <div className="flex items-baseline gap-2">
                  <span className="text-xs font-bold text-indigo-400">AFN</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm overflow-y-auto">
          <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-200 my-8">
            ...
            <form onSubmit={handleSubmit} className="p-8 overflow-y-auto max-h-[70vh] custom-scrollbar">
              ...
              <SectionHeader title="ویزه و خدمات ویژه" />
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  <label className="text-xs font-bold text-slate-400 block text-right">هزینه ویزه (AFN)</label>
                  <input required name="visaCost" type="text" value={formData.visaCost} onChange={handleInputChange} placeholder="۱۵,۰۰۰" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all outline-none" />
                </div>
              </div>
              ...
              <div className="space-y-4">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div className="space-y-1">
                    ...
                    <input required name="hotelNights" type="number" value={formData.hotelNights} onChange={handleInputChange} placeholder="۱۵" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all outline-none text-center" />
                  </div>
                  <div className="space-y-1">
                    <label className="text-xs font-bold text-slate-400 block text-right">نوع اتاق</label>
                    <input required name="hotelRooms" value={formData.hotelRooms} onChange={handleInputChange} placeholder="اتاق ۲ نفره" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all outline-none" />
                  </div>
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <input required name="foodDuration" type="number" value={formData.foodDuration} onChange={handleInputChange} placeholder="۱۵" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all outline-none text-center" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <input required name="flightPrice" type="text" value={formData.flightPrice} onChange={handleInputChange} placeholder="۵۲,۰۰۰" className="w-full px-5 py-4 bg-slate-50 border border-slate-100 rounded-2xl focus:ring-2 focus:ring-indigo-500 transition-all outline-none" />
                </div>
              </div>

              <div className="mt-12 bg-indigo-50/50 p-8 rounded-[32px] border border-indigo-100 grid grid-cols-2 gap-8">
                <div className="space-y-1">
                  <label className="text-xs font-black text-indigo-400 block text-right uppercase">سود خالص (AFN)</label>
                  <input required name="profit" value={formData.profit} onChange={handleInputChange} placeholder="۲۲,۰۰۰" className="w-full px-4 py-3 bg-white border border-indigo-100 rounded-2xl text-xl font-black text-emerald-600 focus:ring-2 focus:ring-emerald-500 transition-all outline-none" />
                </div>
                <div className="space-y-1 text-left">
                  <label className="text-xs font-black text-indigo-400 block text-left uppercase">قیمت کل برای مشتری</label>
                  <input required name="totalPrice" value={formData.totalPrice} onChange={handleInputChange} placeholder="۱۳۹,۰۰۰" className="w-full px-4 py-3 bg-white border border-indigo-100 rounded-2xl text-xl font-black text-indigo-600 focus:ring-2 focus:ring-indigo-500 transition-all outline-none text-left" />
                </div>
              </div>
