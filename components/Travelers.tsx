...
const Travelers: React.FC<TravelersProps> = ({ companies, travelers, onUpdateTravelers, onUpdateCompanies }) => {
  ...
  useEffect(() => {
    ...
    const vSelling = parse(formData.visaSelling);
    const rFee = parse(formData.representativeFee);
    
    let calculatedTotal = vSelling + rFee;
  ...
  const startEdit = (traveler: Traveler) => {
    ...
    setFormData({
      ...
      tripType: traveler.tripType || 'حج',
      tripDate: traveler.tripDate || '1403/05/20',
      saudiRepresentative: traveler.saudiRepresentative || '',
      representativeFee: traveler.representativeFee || '',
      specialType: traveler.specialType || 'ویزه بی خدمات',
      visaProviderCompany: traveler.visaProviderCompany || '',
      visaPurchase: traveler.visaPurchase || '',
      visaSelling: traveler.visaSelling || '',
      flightRoute: traveler.flightRoute || 'کابل-جده',
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Helper to generate service ledger entries
    const createServiceLedger = (timestamp: number) => {
      const entries: LedgerEntry[] = [];
      if (parseVal(formData.visaSelling) > 0) {
        entries.push({
          id: `${timestamp}-visa`,
          date: currentDate,
          description: `هزینه ویزه (${formData.specialType})`,
          debit: parseVal(formData.visaSelling),
          credit: 0
        });
      }
      if (formData.specialType === 'ویزه با خدمات') {
        if (parseVal(formData.flightSelling) > 0) {
          entries.push({ id: `${timestamp}-flight`, date: currentDate, description: `فروش تکت (${formData.flightRoute})`, debit: parseVal(formData.flightSelling), credit: 0 });
        }
        const madinahTotal = parseVal(formData.hotelSelling) * (parseFloat(formData.hotelNights) || 1);
        if (madinahTotal > 0) {
          entries.push({ id: `${timestamp}-hotel-madinah`, date: currentDate, description: `فروش هوتل مدینه (${formData.hotelName} - ${formData.hotelNights} شب)`, debit: madinahTotal, credit: 0 });
        }
        const makkahTotal = parseVal(formData.hotelMakkahSelling) * (parseFloat(formData.hotelMakkahNights) || 1);
        if (makkahTotal > 0) {
          entries.push({ id: `${timestamp}-hotel-makkah`, date: currentDate, description: `فروش هوتل مکه (${formData.hotelMakkahName} - ${formData.hotelMakkahNights} شب)`, debit: makkahTotal, credit: 0 });
        }
        if (parseVal(formData.transportSelling) > 0) {
          entries.push({ id: `${timestamp}-transport`, date: currentDate, description: `فروش ترانسپورت (${formData.transportType})`, debit: parseVal(formData.transportSelling), credit: 0 });
        }
      }
      if (parseVal(formData.representativeFee) > 0) {
        entries.push({ id: `${timestamp}-rep-fee`, date: currentDate, description: `هزینه خدمات نماینده (${formData.saudiRepresentative})`, debit: parseVal(formData.representativeFee), credit: 0 });
      }
      return entries;
    };
    ...
    if (editingTravelerId) {
      onUpdateTravelers(travelers.map(t => {
        if (t.id === editingTravelerId) {
          // Keep only non-automatic ledger entries (payments or manual adjustments)
          const preservedLedger = t.ledger.filter(entry => 
            !entry.description.includes('هزینه ویزه') &&
            !entry.description.includes('فروش تکت') &&
            !entry.description.includes('فروش هوتل') &&
            !entry.description.includes('فروش ترانسپورت') &&
            !entry.description.includes('هزینه خدمات نماینده') &&
            !entry.description.includes('دریافت نقد اولیه')
          );
          
          // Re-generate service entries based on updated form
          const updatedServiceLedger = createServiceLedger(Date.now());
          
          return { 
            ...t, 
            ...formData, 
            name: travelerName,
            ledger: [...updatedServiceLedger, ...preservedLedger]
          };
        }
        return t;
      }));
      ...
      const initialServiceLedger = createServiceLedger(timestamp);
      const received = parseVal(formData.totalReceived);
      
      const initialPaymentEntry: LedgerEntry = {
      ...
      const newEntry: Traveler = {
        ...
        status: 'در انتظار',
        date: currentDate,
        ledger: [...initialServiceLedger, initialPaymentEntry]
      };
  ...
  const closeModal = () => {
    ...
    setFormData({
      ...
      saudiRepresentative: '', representativeFee: '', specialType: 'ویزه بی خدمات', 
      visaProviderCompany: '',
      visaPurchase: '', visaSelling: '', flightRoute: 'کابل-جده', flightPurchase: '',
      ...
      <div className="bg-white p-4 rounded-3xl shadow-sm border border-slate-100 flex flex-col md:flex-row gap-4 items-center">
        <div className="relative flex-1 w-full">
          ...
          <input 
            ...
            className="w-full pr-12 pl-4 py-3 bg-slate-50 border border-slate-200 rounded-xl focus:outline-none focus:ring-2 focus:ring-[#108548] transition-all font-bold"
            value={searchTerm}
          ...
          <table className="w-full text-right border-collapse">
            ...
            <tbody className="divide-y divide-slate-50">
              {filteredTravelers.map((traveler) => {
                const totalDebit = traveler.ledger.reduce((sum, entry) => sum + (entry.debit || 0), 0);
                const totalCredit = traveler.ledger.reduce((sum, entry) => sum + (entry.credit || 0), 0);
                ...
                return (
                  <tr key={traveler.id} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="px-6 py-5 text-sm font-bold text-slate-600">{traveler.phone}</td>
                    <td className="px-6 py-5 text-sm text-slate-500">{traveler.date}</td>
                    <td className="px-6 py-5">
                    ...
                    <td className="px-6 py-5 relative text-center">
                      <button 
      ...
      {isLedgerOpen && selectedTravelerForLedger && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-4xl rounded-[40px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col max-h-[90vh]">
            ...
            <div className="flex-1 overflow-y-auto p-8 space-y-8 text-right" dir="rtl">
              <div className="grid grid-cols-3 gap-6">
                <div className="bg-rose-50 p-6 rounded-3xl border border-rose-100">
                  ...
                  <span className="text-2xl font-black text-rose-600">
                      {selectedTravelerForLedger.ledger.reduce((sum, e) => sum + (e.debit || 0), 0).toLocaleString('en-US')} $
                  </span>
                </div>
                <div className="bg-emerald-50 p-6 rounded-3xl border border-emerald-100">
                  <span className="text-xs font-bold text-emerald-400 block mb-1">مجموع رسید (پرداختی)</span>
                  <span className="text-2xl font-black text-emerald-600">
                      {selectedTravelerForLedger.ledger.reduce((sum, e) => sum + (e.credit || 0), 0).toLocaleString('en-US')} $
                  </span>
                </div>
                <div className="bg-slate-900 p-6 rounded-3xl text-white">
                  <span className="text-xs font-bold text-slate-400 block mb-1">باقیمانده حساب</span>
                  <span className="text-2xl font-black">
                    {(selectedTravelerForLedger.ledger.reduce((sum, e) => sum + (e.debit || 0), 0) - 
                      selectedTravelerForLedger.ledger.reduce((sum, e) => sum + (e.credit || 0), 0)).toLocaleString('en-US')} $
                  </span>
                </div>
              </div>
              ...
              <form onSubmit={handleAddLedgerEntry} className="bg-slate-50 p-6 rounded-[32px] border border-slate-100 flex flex-wrap gap-4 items-end">
                <div className="flex-1 min-w-[200px] space-y-1.5">
                  ...
                  <input required name="description" value={ledgerFormData.description} onChange={handleLedgerInputChange} className="w-full px-5 py-3 bg-white border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-bold" />
                </div>
                <div className="w-40 space-y-1.5">
                  <label className="text-xs font-black text-slate-500 mr-2">مبلغ برد ($)</label>
                  <input type="number" name="debit" value={ledgerFormData.debit} onChange={handleLedgerInputChange} className="w-full px-5 py-3 bg-white border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-black text-rose-600" />
                </div>
                <div className="w-40 space-y-1.5">
                  <label className="text-xs font-black text-slate-500 mr-2">مبلغ رسید ($)</label>
                  <input type="number" name="credit" value={ledgerFormData.credit} onChange={handleLedgerInputChange} className="w-full px-5 py-3 bg-white border border-slate-200 rounded-2xl focus:ring-2 focus:ring-indigo-500 outline-none font-black text-emerald-600" />
                </div>
                <button type="submit" className="bg-indigo-600 hover:bg-indigo-700 text-white p-4 rounded-2xl transition-all">
                  <Plus size={24} />
                </button>
              </form>
                ...
                <table className="w-full text-right">
                  <thead className="bg-slate-50">
                    <tr>
                      ...
                      <th className="px-6 py-4 text-sm font-black text-rose-500">برد ($)</th>
                      <th className="px-6 py-4 text-sm font-black text-emerald-600">رسید ($)</th>
                      ...
                      return selectedTravelerForLedger.ledger.map((entry) => {
                        ...
                        return (
                          <tr key={entry.id} className="hover:bg-slate-50/50 transition-colors">
                            ...
                            <td className="px-6 py-4 font-black text-slate-800">{runningBalance.toLocaleString('en-US')} $</td>
                          </tr>
                        );
                      });
                    })()}
            ...
            <div className="p-8 border-t border-slate-50 flex justify-between">
               ...
               <button onClick={() => setIsLedgerOpen(false)} className="px-10 py-3 bg-slate-900 text-white rounded-xl font-black">بستن</button>
            </div>
          </div>
        </div>
      )}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-sm">
          <div className="bg-white w-full max-w-2xl rounded-[40px] shadow-2xl overflow-hidden animate-in fade-in zoom-in duration-300 flex flex-col max-h-[90vh]">
            <div className="flex items-center justify-between p-8 border-b border-slate-50 bg-[#108548]/5">
              <div className="flex items-center gap-4">
                <div className="w-14 h-14 bg-[#108548] text-white rounded-2xl flex items-center justify-center">
                  <User size={28} />
                </div>
                <div>
                  <h3 className="text-2xl font-black text-slate-800">{selectedTravelerForDetails.name}</h3>
                  <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{selectedTravelerForDetails.code}</p>
                </div>
              </div>
              <button onClick={() => setIsDetailsOpen(false)} className="p-3 hover:bg-slate-100 rounded-full text-slate-400 transition-colors">
                <X size={24} />
              </button>
            </div>
            ...
            <div className="p-8 border-t border-slate-50 flex justify-end">
               <button onClick={() => setIsDetailsOpen(false)} className="px-10 py-3 bg-[#108548] text-white rounded-xl font-black">بستن</button>
            </div>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleSubmit} className="p-8 overflow-y-auto max-h-[85vh] custom-scrollbar text-right" dir="rtl">
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <input required name="firstName" value={formData.firstName} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block font-bold">تخلص</label>
                  <input required name="lastName" value={formData.lastName} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block font-bold">شماره پاسپورت</label>
                  <input required name="passport" value={formData.passport} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block font-bold">شماره تماس</label>
                  <input required name="phone" value={formData.phone} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select name="tripType" value={formData.tripType} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold">
                ...
                <div className="space-y-1">
                  ...
                  <input name="tripDate" value={formData.tripDate} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold text-center" />
                </div>
              </div>
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className="space-y-1">
                  ...
                  <select name="specialType" value={formData.specialType} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold">
                    <option value="ویزه بی خدمات">ویزه بی خدمات</option>
                    <option value="ویزه با خدمات">ویزه با خدمات</option>
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block font-bold">شرکت تأمین‌کننده ویزه</label>
                  <select name="visaProviderCompany" value={formData.visaProviderCompany} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold">
                    <option value="">انتخاب شرکت...</option>
                    {companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                  </select>
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block font-bold">قیمت خرید ویزه ($)</label>
                  <input name="visaPurchase" value={formData.visaPurchase} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                </div>
                <div className="space-y-1">
                  <label className="text-xs text-slate-400 block font-bold">قیمت فروش ویزه ($)</label>
                  <input name="visaSelling" value={formData.visaSelling} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                </div>
              </div>
                  ...
                  <>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1 col-span-2">
                        ...
                        <input name="flightRoute" value={formData.flightRoute} onChange={handleInputChange} placeholder="کابل-جده-کابل" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold" />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs text-slate-400 block font-bold">قیمت خرید ($)</label>
                        <input name="flightPurchase" value={formData.flightPurchase} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs text-slate-400 block font-bold">قیمت فروش ($)</label>
                        <input name="flightSelling" value={formData.flightSelling} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                      </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                          ...
                          <select name="hotelName" value={formData.hotelName} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold">
                            <option value="">انتخاب شرکت...</option>
                            {companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                          </select>
                        </div>
                        <div className="space-y-1">
                          ...
                          <input name="hotelNights" value={formData.hotelNights} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                        </div>
                        <div className="space-y-1">
                          <label className="text-xs text-slate-400 block font-bold">خرید هر شب ($)</label>
                          <input name="hotelPurchase" value={formData.hotelPurchase} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                        </div>
                        <div className="space-y-1">
                          <label className="text-xs text-slate-400 block font-bold">فروش هر شب ($)</label>
                          <input name="hotelSelling" value={formData.hotelSelling} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                          ...
                          <select name="hotelMakkahName" value={formData.hotelMakkahName} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold">
                            <option value="">انتخاب شرکت...</option>
                            {companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                          </select>
                        </div>
                        <div className="space-y-1">
                          ...
                          <input name="hotelMakkahNights" value={formData.hotelMakkahNights} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                        </div>
                        <div className="space-y-1">
                          <label className="text-xs text-slate-400 block font-bold">خرید هر شب مکه ($)</label>
                          <input name="hotelMakkahPurchase" value={formData.hotelMakkahPurchase} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                        </div>
                        <div className="space-y-1">
                          <label className="text-xs text-slate-400 block font-bold">فروش هر شب مکه ($)</label>
                          <input name="hotelMakkahSelling" value={formData.hotelMakkahSelling} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                          ...
                          <select name="transportCompany" value={formData.transportCompany} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold">
                            <option value="">انتخاب شرکت...</option>
                            {companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                          </select>
                        </div>
                        <div className="space-y-1">
                          ...
                          <select name="transportType" value={formData.transportType} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold">
                        ...
                        <div className="space-y-1">
                          ...
                          <input name="transportPurchase" value={formData.transportPurchase} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                        </div>
                        <div className="space-y-1">
                          <label className="text-xs text-slate-400 block font-bold">فروش ترانسپورت ($)</label>
                          <input name="transportSelling" value={formData.transportSelling} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                        </div>
                    </div>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                      <div className="space-y-1">
                        <label className="text-xs text-slate-400 block font-bold">انتخاب نماینده</label>
                        <select name="saudiRepresentative" value={formData.saudiRepresentative} onChange={handleInputChange} className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none font-bold">
                          <option value="">انتخاب نماینده...</option>
                          {companies.map(c => <option key={c.id} value={c.name}>{c.name}</option>)}
                        </select>
                      </div>
                      <div className="space-y-1">
                        <label className="text-xs text-slate-400 block font-bold">هزینه خدمات نماینده ($)</label>
                        <input name="representativeFee" value={formData.representativeFee} onChange={handleInputChange} placeholder="0" className="w-full px-4 py-2 bg-slate-50 border border-slate-100 rounded-xl focus:ring-2 focus:ring-[#108548] outline-none text-center font-bold" />
                      </div>
                    </div>
                  </>
              )}
              ...
              <div className="mt-12 flex flex-col md:flex-row items-center justify-between gap-4 pt-6 border-t border-slate-50">
                <div className="flex flex-col gap-4 w-full md:w-auto">
                  <div className="flex items-center justify-between gap-4">
                    ...
                    <input readOnly name="totalPayable" value={formData.totalPayable} className="w-32 px-3 py-1 bg-slate-100 border border-slate-200 rounded-lg outline-none text-center font-black text-slate-800" />
                  </div>
                  <div className="flex items-center justify-between gap-4">
                    <label className="text-xs font-black text-slate-500">مجموع دریافتی فعلی:</label>
                    <input name="totalReceived" value={formData.totalReceived} onChange={handleInputChange} placeholder="0" className="w-32 px-3 py-1 bg-slate-50 border border-slate-100 rounded-lg focus:ring-2 focus:ring-[#108548] outline-none text-center font-black text-emerald-600" />
                  </div>
                </div>
